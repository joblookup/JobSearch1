...
def main ():
    ...
    whatJob = info.get('q')
    where = info.get('l')
    url4 = urllib.request.urlopen("https://www.indeed.com/jobs?q=" + whatJob + "&l=" + where + "%2C+CA")
    content3 = url4.read()
    ...
    while soup3.findAll('div', {'class': 'invalid_location'}):
        ...
        content3 = url4.read()
        soup3 = BeautifulSoup(content3, "html.parser")
        time = input('What would you like to work as? ')
        #time = "internship"
        time = time.lower()
        if (time == "full-time"):
            time = "fulltime"
    ...
    url4 = "https://www.indeed.com/jobs?q=" + whatJob + "+$" + salaryReturn1 + "&l=" + where + ",+CA&jt=" + time
